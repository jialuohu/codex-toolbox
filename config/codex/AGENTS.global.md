...
Use Context7 for current, version-aware library and framework documentation, API references, migration examples, and code examples from package docs. Prefer Context7 over general web search when the task is about how to use a specific library or framework. Use built-in Codex web search for remaining broad public discovery. For OpenAI/Codex product behavior, prefer official OpenAI/Codex docs before Context7 or generic web search.

Use built-in Codex web search by default for ordinary public discovery, current facts, documentation, news, and citations. Use Firecrawl only when the task needs full-page clean Markdown, JavaScript rendering, structured extraction, site mapping or crawling, monitoring, or extraction from a selected page that normal search or a direct read could not retrieve adequately. For a straightforward user-supplied article URL, prefer Defuddle. For a known URL that needs Firecrawl, call `firecrawl_scrape` instead of searching again. If Firecrawl search is materially useful, search first without `scrapeOptions`, use a result limit of 5 or less, and then scrape only the selected URLs. Every map or crawl must have an explicit page limit. Use Firecrawl Interact or Agent only when the user explicitly needs those capabilities or the cheaper bounded routes failed. If Firecrawl tools are not visible for one of these justified cases, call `tool_search` for the exact capability. After using `firecrawl_search`, call the Firecrawl feedback tool only when that tool is exposed. Do not use Firecrawl for the user's private local files, saved Zotero library, Obsidian vault content, or other private workspace data unless the user explicitly asks to send that content to Firecrawl.

Use `$wechat-digest` for selected WeChat subscriptions. For latest/current/recent configured-source questions, use its read-only interactive route: `configured-sources`, then `latest`, `recent`, or `read` by configured source ID. If an exact source name is not cached, use exact `search-sources`, require exactly one result whose ID is configured, then use the interactive command by ID. This route takes precedence over the general web routing. Use Firecrawl only when interactive `read` returns its validated structured fallback, with no claim/renew/ack gates, or within the preserved incremental digest claim/renew/ack lifecycle. Do not substitute generic search or scraping for configured-source interaction, subscription scanning, baseline state, deduplication, or acknowledgment. For a user-supplied standalone article URL, prefer Defuddle and use Firecrawl only when the validated page needs difficult or dynamic extraction; for a historical/topic search that does not depend on subscription state, use built-in Codex web search.
...
Use `$paper-library-intake` when one paper named by title, DOI, arXiv ID/URL, or publisher URL must be found, checked against Zotero, classified, added, or repaired. Search Zotero first for the private-library state, use Paper Search first for public scholarly discovery, cross-source validation, and lawful open-access retrieval, and use normal Codex web search when a current canonical public page is still needed. Use Firecrawl only when a selected canonical paper page requires clean or dynamic extraction. `find`, `check`, and `explain` are read-only. An explicit `add`, `save`, or `import` request authorizes only that paper's parent item, lawful attachment, suitable topical memberships, `Research/ReadLater`, and the skill's bounded missing-topic creation; it does not authorize merge, delete, semantic indexing, or unrelated cleanup. Follow the skill's WebDAV/Zotero-Storage preflight and readable-PDF completion rule.

Use `paper_search_mcp` for structured academic discovery, cross-source validation, and lawful open-access retrieval across arXiv, Semantic Scholar, CORE, IEEE, Unpaywall, CrossRef, PubMed, BASE, and related sources. In the paper-library intake workflow, Paper Search is the first public scholarly discovery and retrieval layer; normal Codex web search and then Firecrawl extraction are bounded fallbacks for canonical public pages. The toolbox disables direct Sci-Hub and the upstream generic fallback whose default enables Sci-Hub; prefer source-native OA downloads. If another generic fallback is present, always set `use_scihub=false`.
...
Use `$zotero-todoist-reading-tasks` when a saved Zotero collection or item should become a Todoist paper-reading task, readings need to be distributed across dates, or existing reading tasks need Zotero Desktop links repaired. Keep Zotero read-only, fetch attachment children in batches, use the parent item key for `zotero://select` and the PDF attachment key for `zotero://open-pdf`, and stop when multiple PDFs lack an authoritative primary attachment. Prefer the connected Todoist app and use the hosted MCP only as a fallback; choose one Todoist surface per request. Deduplicate by the parent-item URI, preserve unrelated task fields, use Todoist due dates for planned reading days and `deadlineDate` for the final cutoff, and never claim continuous synchronization from a one-time reconciliation. Its default PaperRead coupling calls `$paper-read-draft` exactly once per uniquely resolved Zotero parent before that parent's first Todoist write; `$paper-read-draft` is the sole owner of note identity, safe creation, and URI generation. Use only its returned URI for one canonical managed Obsidian line. An explicit `without Obsidian notes` opts out completely and preserves existing Obsidian lines. For unavailable or URI-less PaperRead results, continue valid Todoist work best-effort as `note-missing`, removing only canonical managed Obsidian lines and preserving noncanonical content for manual review. The `obsidian_files` dependency belongs to `obsidian-tools`; do not duplicate its MCP configuration in `research-tools`.
...
Use `$daily-command-center` for bounded cross-app reads that prepare a daily brief from Gmail, Google Calendar, and Todoist. Gmail is incoming context; Todoist is the durable source of truth for actionable tasks; Google Calendar is the source of truth for time commitments; conversation history is not a task database. Keep strict no-mutation behavior in scheduled runs and manual briefs: do not change email, Calendar, or Todoist records. Prefer connected apps; for Todoist use the connected Todoist app first and the official hosted MCP as a Codex CLI fallback, never both in the same run. If a source is unavailable, finish a partial brief, name the missing source and bounded coverage, and do not substitute web search.
