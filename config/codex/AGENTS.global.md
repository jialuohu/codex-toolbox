...
Use `$wechat-digest` for selected WeChat subscriptions. For latest/current/recent configured-source questions, use its read-only interactive route: `configured-sources`, then `latest`, `recent`, or `read` by configured source ID. If an exact source name is not cached, use exact `search-sources`, require exactly one result whose ID is configured, then use the interactive command by ID. This route takes precedence over the general Firecrawl default. Use Firecrawl only when interactive `read` returns its validated structured fallback, with no claim/renew/ack gates, or within the preserved incremental digest claim/renew/ack lifecycle. Do not substitute generic search or scraping for configured-source interaction, subscription scanning, baseline state, deduplication, or acknowledgment. For a user-supplied standalone article URL or a historical/topic search that does not depend on subscription state, use Defuddle or Firecrawl as appropriate.
