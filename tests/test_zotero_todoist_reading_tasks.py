...
class ZoteroTodoistReadingTasksContractTests(unittest.TestCase):
    ...
    def test_skill_manages_the_exact_obsidian_line_without_touching_other_content(self) -> None:
        ...
        for expected in (
            ...
            "an `obsidian://open` target with exactly two non-empty query parameters",
            "exactly two non-empty query parameters in that order: `vault`, then `file`",
            "no additional query parameters, no fragment, and no extra line text",
            "no extra line text",
            ...
            "reversed query parameter order",
    ...
    def test_plugin_docs_and_setup_checker_expose_the_workflow(self) -> None:
        ...
        for path, expected in (
            ...
            (SETUP_CHECKER, 'research_plugin.get("version") == "0.8.3"'),
