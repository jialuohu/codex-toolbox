...
5. First, prepare a complete article output block with WeChat source name, title, canonical URL, publication time when present, content source, and concise summary. On success, then call `ack <article_id>`, then include the prepared block in the final digest; do not end the scheduled task before the acknowledgment. On any fetch, fallback, or summary failure, call `fail <article_id> --reason <SAFE_CODE>` and leave it pending or exhausted. After three failures, do not ack or retry it automatically.

Keep the helper's lightweight local JSON state boundary; never add content archives or full-text storage.

## Output

For each completed item, emit WeChat source name, title, canonical URL, publication time when present, content source (`bestblogs` or `firecrawl`), and a concise summary. After processing, run `status` directly: `run_wechat_digest.sh status`. End every run with a health footer: scan completeness and warnings, configured/initialized sources, retryable and exhausted counts, `body_budget` day, used, and limit, API-call counters, fallback/failure receipts, and whether the first-run baseline was established.
