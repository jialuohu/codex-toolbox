...
description: Plan and operate Codex plus Symphony plus Linear orchestration. Use when the user asks about Symphony, durable multi-ticket Codex execution, Linear-backed task decomposition, Symphony pilots, planner-side Linear issue creation, Symphony MCP tools, worker status monitoring, or handoff/closeout workflows.
---
...
one-session exploration, or ambiguous planning.

Use Symphony only when the work clearly needs durable multi-ticket execution:
...
4. Confirm the repo has useful tests or review evidence before creating runnable issues.
5. Dry-run issue creation first.
6. Review payloads for project slug, team key, labels, state, acceptance criteria, and verification.
7. Create live Linear issues only after payload review.
8. Start or refresh the Symphony daemon only after the intended ready issues exist.
9. Monitor `/api/v1/state` for running count, issue identifiers, phase, retry rows, and workspace
   paths.
10. Review handoffs manually unless the workflow explicitly enables draft PR handoff.
11. Apply only reviewed files, run verification, commit/push if appropriate, comment evidence back to
...
- Keep Symphony out of vague, high-judgment, fast interactive, or single-task work.
