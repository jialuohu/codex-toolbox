...
if [[ -z "$NODE_BIN" ]]; then
  print -u2 "Node.js is unavailable; rerun the full codex-toolbox setup"
  exit 127
fi

if ! "$NODE_BIN" "$VERIFY_SCRIPT" "$RUNTIME_DIR" "$LOCK_FILE" >/dev/null; then
  print -u2 "Draw.io runtime is missing or invalid; rerun scripts/setup-drawio-tools.sh --install from the toolbox checkout"
  exit 1
fi
