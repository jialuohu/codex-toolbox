...
1. Check `typesafe_status` before a Jev call. The wrapper has no billing setting or spending cap. Do not bypass a readiness block through another client merely to run the pilot. The pilot may remain unrun while offline preparation is complete.
...
5. Collect real baseline and Jev outputs for every case. Measure all input/output tokens and Jev call counts. Record total and Jev charges when available; use JSON `null` for unavailable monetary charges and explain unavailable charge data in run notes. Do not infer invoiced charges from token usage or substitute zero. Do not invent a Jev result when readiness blocks a request. Missing or incomplete pairs are not a completed pilot.
